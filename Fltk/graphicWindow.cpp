...
static void mat_edit_menu(Fl_Widget *w, void *data);
static void calculix_func(Fl_Widget *w, void *data);
static void read_sc_mat(Fl_Widget *w, void *data);

static void init_matlist(){
  //Prof. Yu initial materials list
  ...
  //Matname -> clear();
  //Matname -> add("Select material:", 0, 0, 0);
  /* matname.erase(matname.begin(),matname.end());
  //Matname -> clear();
  ...
  for (int i = 0; i < matname.size(); i++) {
    Matname -> add(matname[i].c_str(), 0, 0, 0);
  }*/
  FlGui::instance()->geoContext7_1->choice[5]->menu(Matname);
  FlGui::instance()->geoContext7_1->choice[6]->menu(Matname);
  FlGui::instance()->geoContext7_1->choice[7]->menu(Matname);
  FlGui::instance()->geoContext7->choice[8]->menu(Matname);
  FlGui::instance()->geoContext7->choice[9]->menu(Matname);
  FlGui::instance()->geoContext14->choice[0]->menu(Matname);
  FlGui::instance()->geoContext14_1->choice[0]->menu(Matname);
  FlGui::instance()->geoContext14_1->choice[1]->menu(Matname);
  FlGui::instance()->geoContext14_1->choice[2]->menu(Matname);
  FlGui::instance()->geoContext14_1->choice[3]->menu(Matname);
  FlGui::instance()->geoContext14_1->choice[4]->menu(Matname);
  FlGui::instance()->geoContext14_1->choice[5]->menu(Matname);
  FlGui::instance()->geoContext14_1->choice[6]->menu(Matname);
  FlGui::instance()->geoContext14_1->choice[7]->menu(Matname);
  FlGui::instance()->geoContext13->choice[0]->menu(Matname);
  FlGui::instance()->geoContext13->choice[1]->menu(Matname);
  FlGui::instance()->geoContext13->choice[2]->menu(Matname);
  FlGui::instance()->geoContext13->choice[3]->menu(Matname);
  FlGui::instance()->geoContext13->choice[4]->menu(Matname);
  FlGui::instance()->geoContext13_2->choice[0]->menu(Matname);
  FlGui::instance()->geoContext13_3->choice[0]->menu(Matname);
  FlGui::instance()->geoContext13_3->choice[1]->menu(Matname);
  FlGui::instance()->geoContext13_3->choice[2]->menu(Matname);
  FlGui::instance()->geoContext13_3->choice[3]->menu(Matname);
  FlGui::instance()->geoContext13_3->choice[4]->menu(Matname);
  FlGui::instance()->geoContext13_3->choice[5]->menu(Matname);
  FlGui::instance()->geoContext13_3->choice[6]->menu(Matname);
  FlGui::instance()->geoContext13_3->choice[7]->menu(Matname);
  FlGui::instance()->geoContext13_5->choice[0]->menu(Matname);
  FlGui::instance()->geoContext13_8->choice[0]->menu(Matname);
  FlGui::instance()->geoContext13_6->choice[0]->menu(Matname);
  FlGui::instance()->geoContext13_6->choice[1]->menu(Matname);
  FlGui::instance()->geoContext13_6->choice[2]->menu(Matname);
  FlGui::instance()->geoContext13_6->choice[3]->menu(Matname);
  FlGui::instance()->geoContext13_6->choice[4]->menu(Matname);
  FlGui::instance()->geoContext13_6->choice[5]->menu(Matname);
  FlGui::instance()->geoContext13_6->choice[6]->menu(Matname);
  FlGui::instance()->geoContext13_6->choice[7]->menu(Matname);
  FlGui::instance()->geoContext13_9->choice[0]->menu(Matname);
  FlGui::instance()->geoContext13_9->choice[1]->menu(Matname);
  FlGui::instance()->geoContext13_9->choice[2]->menu(Matname);
  FlGui::instance()->geoContext13_9->choice[3]->menu(Matname);
  FlGui::instance()->geoContext13_9->choice[4]->menu(Matname);
  FlGui::instance()->geoContext13_9->choice[5]->menu(Matname);
  FlGui::instance()->geoContext13_9->choice[6]->menu(Matname);
  FlGui::instance()->geoContext13_9->choice[7]->menu(Matname);
...
static void file_new_cb(Fl_Widget *w, void *data)
{
  ...
  TCHAR name1 [ UNLEN + 1 ];
  DWORD size = UNLEN + 1;
  std::string wholename = "C:\\Users\\";
  GetUserName( (TCHAR*)name1, &size );
  wholename = wholename + name1 + "\\documents\\new.geo";
  //wholename = wholename + name;
 test:
  if(fileChooser(FILE_CHOOSER_CREATE, "New", "", wholename.c_str())) {
    ...
    init_matlist();
    OpenProject(name);
    std::string rvname = GModel::current()->getFileName();
    std::string t2 = rvname+".t2";
    std::string t3 = rvname+".t3";
    std::string t1 = rvname+".t";
    std::string t4 = rvname+".t4";
    std::string msh = rvname+".msh";
    int findlast = rvname.find_last_of(".");
    std::string fname = rvname.substr(0,findlast);
     std::string msh_opt = fname+".msh.opt";
    std::string msh_ech = fname+".msh.ech";
    std::string viewmsh = fname + "VIEW.msh";
    std::string viewgeo = fname + "VIEW.geo";
    std::string mmsh = fname + ".msh";
    remove(t1.c_str());remove(t2.c_str());remove(t3.c_str());remove(viewmsh.c_str());
    remove(t4.c_str());remove(msh.c_str());remove(viewgeo.c_str());
    remove(msh_opt.c_str());remove(msh_ech.c_str());remove(mmsh.c_str());
    FlGui::instance()->geoContext1->input[0]->value("1");
    FlGui::instance()->geoContext1->input[39]->value("1");
    FlGui::instance()->geoContext1->input[41]->value("1");
    FlGui::instance()->geoContext1->input[40]->value("MAT1");
    FlGui::instance()->geoContext1->input[42]->value("MAT1");
    FlGui::instance()->geoContext1->input[43]->value("MAT1");
    change_period_value(0);
    change_com_value(0);
    change_mat_value(1);
    change_extru_value(0);
    change_layname_value(0);
    change_boxlay_value(0);
    change_Ilay_value(0);
    change_scelem_value(0);
    mesh_chec_cal ini_mb;
    int meanless = ini_mb.re0();
    ini_mb.get_assig(0);
...
//Prof Yu, add save file function
static void file_save_cb(Fl_Widget *w, void *data){
  std::string defname=GModel::current()->getFileName();
  std::string newname;
  if(defname.find("untitled.geo")==std::string::npos) return;
 test:
  if(fileChooser(FILE_CHOOSER_CREATE, "New", "")) {
    std::string name = fileChooserGetName(1);
    newname=name;
  ...
  std::ifstream rfile(defname.c_str());
  std::ofstream ifile(newname.c_str());
  std::string line;
  while(getline(rfile,line)){
    ifile << line << "\n";
  }
  rfile.close();ifile.close();
...
void file_quit_cb(Fl_Widget *w, void *data)
{
  ...
  std::string defname=GModel::current()->getFileName();
  if (defname.length()>8){
    if (defname.find("untitled")!=std::string::npos){
      if(!fl_choice("Do you want to save the model before you leave?",
		    "No", "Yes",0))
	Msg::Exit(0);
      else 
	return;
...
void geometry_reload_cb(Fl_Widget *w, void *data)
{
  ...
  OpenProject(fileName);
  //init_matlist(); //Prof. Yu
  drawContext::global()->draw();
  Msg::StatusBar(true, "Reload done");
...
static void geometry_elementary_add_new_cb(Fl_Widget *w, void *data)
{
  ...
  // Prof. Yu, remind user to create new database
  std::string fileName = GModel::current()->getFileName();
  /* std::string def_name;
  if(fileName.length()<12)
    def_name=fileName;
  else
    def_name=fileName.substr(fileName.length() - 12, fileName.length());
  std::string untitle = "untitled.geo";
  if (def_name == untitle){
    fl_message("Please create a new database!");
    return;
    }*/
...
static void action_point_line_surface_volume(int action, int mode, const char *what)
{
  ...
  //Prof. Wenbin Yu, pop assign material window
  if(action == 7){
    FlGui::instance()->geoContext2->show(0);
  }
  //Prof. Wenbin Yu

  if(action == 20){
    fl_message("Please select part.");
  }
  ...
  while(1) {
    if(!List_Nbr(List1))
      ...
      Msg::StatusGl("Select %s\n"
    	...
    	//wenbin yu
	// action 11 and 12 are periodic selection
	if ((action == 11) || (action == 12)) {
		if (!List_Nbr(List1))
			Msg::StatusGl("Select periodic %s\n"
			"[Press 'e' to end selection or 'q' to abort]", str);
		else
			Msg::StatusGl("Select periodic %s\n"
			"[Press 'e' to end selection, 'u' to undo last selection "
	...
	// for 3d unit cell, after select two periodic surfaces, select corresponding boundaries
	if ((action == 12) && ((List_Nbr(List1)-2)>=0)){
		type = ENT_LINE;
		Msg::StatusGl("Select corresponding boundaries\n"
			"[Press 'e' to end selection or 'q' to abort]");
	}
	//wenbin yu, adding ends
    ...
    if(ib == 'e') {
	  ...
	  add_physical(what, List1, GModel::current()->getFileName(), FlGui::instance()->geoContext2->input[0]->value());   //Prof. Wenbin Yu
          break;
        case 8:
	  ...
	  //Prof. Yu
	case 11:
	  add_periodic_line(List1, GModel::current()->getFileName());
	  break;
	case 12:
	  add_periodic_surface(List1, GModel::current()->getFileName());
	  type = ENT_SURFACE; // back to select surface after selecting corresponding boundaries
	  break;
	case 20:
	  //int basenum = 100;
	  ...
	  //scmat_n1 = scmat_n2.str();
	  add_physical(what, List1, GModel::current()->getFileName(), scmat_n2.str());
	  break;
    ...
    if(ib == 'q') {
	  ...
	  //Prof. Wenbin Yu, when user hit q, close assign material window(geometryContextWindow2) at the same time
	  FlGui::instance()->geoContext2->hide(0);

	  //Prof. Wenbin Yu, when user hit q, close refine mesh window(meshContext) at the same time
	  //FlGui::instance()->meshContext->hide(0);
...
{
  if(!data) return;
  //FlGui::instance()->geoContext->show(2);                                      //Prof.Wenbin Yu
  FlGui::instance()->geoContext->show(1);                                      //Prof.Wenbin Yu
...
{
  if(!data) return;
  //FlGui::instance()->geoContext->show(3);                                      //Prof.Wenbin Yu
  FlGui::instance()->geoContext->show(2);                                      //Prof.Wenbin Yu
...
{
  if(!data) return;
  //FlGui::instance()->geoContext->show(4);                                      //Prof.Wenbin Yu
  FlGui::instance()->geoContext->show(3);                                      //Prof.Wenbin Yu
...
{
  if(!data) return;
  //FlGui::instance()->geoContext->show(5);                                      //Prof.Wenbin Yu
  FlGui::instance()->geoContext->show(4);                                      //Prof.Wenbin Yu
...
//Prof. Yu
static void geometry_SG_cb(Fl_Widget *w, void *data)
{
  if (com_flags == 1) {
    fl_message("Model geometry has been created. Please create a new file!");
    return;
  }
  // Prof. Yu, remind user to create new database
  std::string fileName = GModel::current()->getFileName();
  /* std::string def_name = fileName.substr(fileName.length() - 12, fileName.length());
  std::string untitle = "untitled.geo";
  if (def_name == untitle){
    fl_message("Please create a new database!");
    return;
  ...
  std::string str((const char*)data);
  if (str == "Solid")
    FlGui::instance()->geoContext7->show(0);
  else if (str == "Beam")
    FlGui::instance()->geoContext13->show(0);
  else if (str == "2DSG")
    FlGui::instance()->geoContext7_1->show(0);
  else if (str == "Plate")
    FlGui::instance()->geoContext14P->show(0);
...
static void geometry_physical_add_cb(Fl_Widget *w, void *data)
{
  ...
  if(str == "mech")
    FlGui::instance()->geoContext1->show(0);
  else if (str == "therm")
    FlGui::instance()->geoContext21->show(0);
  else if (str == "HomoBeam")
    FlGui::instance()->geoContext8->show(0);
  else if (str == "HomoSP")
    FlGui::instance()->geoContext9->show(0);
  else if (str == "HomoSolid")
    FlGui::instance()->geoContext10->show(0);
  else if (str == "dehomoBeam")
    FlGui::instance()->geoContext11->show(0);
  else if (str == "dehomoSP")
    FlGui::instance()->geoContext12->show(0);
  else if (str == "dehomoSolid")
    FlGui::instance()->geoContext5->show(0);
  else
...
//Prof Yu
//Add import scfile function
/*
static void importSC(Fl_Widget *w, void *data){
  std::string scfile;
  int Smodel=0;
  if(!data) return;
  std::string str((const char*)data);
  if(str == "scfile"){
    ...
    scfile=chooser.value();
 
    if(scfile.substr(scfile.length()-4)==".sc1") Smodel=1;
    else if (scfile.substr(scfile.length()-4)==".sc2") Smodel=2;
    else if (scfile.substr(scfile.length()-4)==".sc3") Smodel=3;
    else
      {fl_message("Please choose '.sc1', '.sc2' or '.sc3' file!");return;}

    run_homo(Smodel,scfile,1);

    std::string gfile=scfile+".geo";
    OpenProject(gfile);
    return;
  }
    ...
    std::string fglb=name.substr(0,name.length()-4)+".msh.glb";
    std::string scfile2=name.substr(0,name.length()-4);
    std::fstream glbfile(fglb.c_str());
    if(!glbfile) {fl_message("Using dehomogenziation to create .glb file first!");return;}
    std::string new_glb=name.substr(0,name.length()-4)+".glb";
    rename(fglb.c_str(),new_glb.c_str());
    run_homo(Smodel,scfile2,2);
    return;//test
    std::string geofile=scfile2.substr(0,scfile2.length()-4)+".msh";

    ImportSC test1(scfile2,geofile,Smodel);
    test1.get_res(scfile2);
...
static void importSC2(Fl_Widget *w, void *data){
  ...
  std::string scfile=chooser.value();
  std::string geofile=scfile+".geo";
  OpenProject(geofile);
  drawContext::global()->draw();
...
std::string checkmesh = GModel::current()->getFileName();


void mesh_1d_cb(Fl_Widget *w, void *data)
{
  std::string temp0= GModel::current()->getFileName();  
  if (temp0.length()>8){
    if (temp0.find("untitled")!=std::string::npos){
      fl_message("Please save the model before meshing!");
      return;
    }
  }

  
  mesh_chec_cal mesh_b2;
  mesh_b2.re1();
  // Prof. Yu, set default options
  std::string fileName = GModel::current()->getFileName();
  if (meshflags != 1 || checkmesh != fileName){
    std::ofstream meshformat;
    meshformat.open(fileName.c_str(), std::ofstream::app);
    meshformat << "Mesh.Points = 1;\n";
    meshformat.close();
    meshflags = 1;
    checkmesh = fileName;
  }
...
void mesh_2d_cb(Fl_Widget *w, void *data)
{
  ...
  std::string fileName = GModel::current()->getFileName();
  std::ifstream strc_mesh(fileName.c_str());
  std::string read;
  int flags = 0;
  std::string temp,temp1, temp2,temp3;
  std::vector<std::string> matmesh;
  std::vector<std::string> readout;
  while (getline(strc_mesh,read)){

    if (read.find("Square Pack Microstructure") != std::string::npos) flags = 1;
	if (flags != 1) readout.push_back(read);
    if (read.find("interpharse") != std::string::npos) flags = 0;
    if (read.find("r =") != std::string::npos){
      std::istringstream radius(read);
      radius >> temp1 >> temp2 >> temp;
    }
    if (read.find("MATID") != std::string::npos){
      std::istringstream matm(read);
      matm >> temp1 >> temp2 >> temp3;
      matmesh.push_back(temp2.c_str());
      matmesh.push_back(temp3.c_str());
    }
  }
  strc_mesh.close();
  if (flags == 1){
    remove(fileName.c_str());
    new_struc_mesh(fileName, readout, temp, matmesh);
  }
  OpenProject(fileName);
  //Prf. Yu, set default mesh options
  //if (checkmesh != fileName ){
  std::ofstream meshformat;
  meshformat.open(fileName.c_str(), std::ofstream::app);
  meshformat << "Mesh.Points = 1;\n";
  meshformat << "Mesh.SurfaceFaces=1;\n";
  meshformat << "Mesh.SurfaceEdges=1;\n";
  meshformat << "Mesh.VolumeEdges=1;\n";
  meshformat << "Mesh.ColorCarousel=2;\n";
  meshformat.close();
  meshflags = 2;
  checkmesh = fileName;
	  //}
...
void mesh_3d_cb(Fl_Widget *w, void *data)
{
  ...
  std::string fileName = GModel::current()->getFileName();
	// Prof. Yu, set default options
  if (meshflags != 3 || checkmesh != fileName){
	  std::ofstream meshformat;
	  meshformat.open(fileName.c_str(), std::ofstream::app);
	  meshformat << "Mesh.Points = 1;\n";
	  meshformat << "Mesh.SurfaceFaces=1;\n";
...
// Prof. Wenbin Yu, add periodic function
static void geo_periodic_cb(Fl_Widget *w, void *data)
{
  if (extru_flags == 1){
    fl_message("Extrude geometry can not assign periodic B.C.!");
    return;
  }
  if (period_flags == 1){
    fl_message("Do not need to define periodic B.C. for Common Model!");
    return;
  }
  action_point_line_surface_volume(11, 0, "Line");
}

static void geo_periodic_surf(Fl_Widget *w, void *data)
{
  if (extru_flags == 1){
    fl_message("Extrude geometry can not assign periodic B.C.!");
    return;
  ...
  action_point_line_surface_volume(12, 0, "Surface");
}
...
static Fl_Menu_Item bar_table[] = {
  ...
  {"M&erge...",   FL_CTRL+FL_SHIFT+'o', (Fl_Callback *)file_open_merge_cb, (void*)"merge"},
    ...
    {"Save...", FL_CTRL+'w', (Fl_Callback *)file_save_cb, 0},
...
static Fl_Menu_Item sysbar_table[] = {
  {"File", 0, 0, 0, FL_SUBMENU},
    ...
    {"Save ...", FL_META+'w', (Fl_Callback *)file_save_cb, 0},
...
static menuItem static_modules[] = {
  ...
  { "0Modules/Material/Thermoelastic",
	(Fl_Callback *)geometry_physical_add_cb, (void*)"mech" } ,
  { "0Modules/Material/Conduction",
	(Fl_Callback *)geometry_physical_add_cb, (void*)"therm" } ,
  /*  { "0Modules/Material/Edit",
      (Fl_Callback *)mat_edit_menu, (void*)"edit" } ,*/
  /*		//Prof. Yu
  { "0Modules/Geometry/Common SG/Beam Model",
    (Fl_Callback *)geometry_SG_cb, (void*)"Beam"},
  { "0Modules/Geometry/Common SG/Plate(Shell) Model",
    (Fl_Callback *)geometry_SG_cb, (void*)"Plate"},
  { "0Modules/Geometry/Common SG/Solid Model",
  (Fl_Callback *)geometry_SG_cb, (void*)"Solid"},*/

  { "0Modules/Geometry/Common SG/ 1D SG",
    (Fl_Callback *)geometry_SG_cb, (void*)"Plate"},
  { "0Modules/Geometry/Common SG/ 2D SG/Beam sections",
    (Fl_Callback *)geometry_SG_cb, (void*)"Beam"},
  { "0Modules/Geometry/Common SG/ 2D SG/Other 2D SGs",
    (Fl_Callback *)geometry_SG_cb, (void*)"2DSG"},
  { "0Modules/Geometry/Common SG/ 3D SG",
    (Fl_Callback *)geometry_SG_cb, (void*)"Solid"},
  ...
  {"0Modules/Geometry/Elementary entities/Delete/Volume",
   ...
   //Prof. Yu
  { "0Modules/Geometry/Assign user-defined material/1D SG",
   (Fl_Callback *)geometry_physical_add_cb, (void*)"Line" },
  { "0Modules/Geometry/Assign user-defined material/2D SG",
   (Fl_Callback *)geometry_physical_add_cb, (void*)"Surface" },
  { "0Modules/Geometry/Assign user-defined material/3D SG",
   (Fl_Callback *)geometry_physical_add_cb, (void*)"Volume" },
  { "0Modules/Geometry/Assign structural properties/Beam",
   (Fl_Callback *)read_sc_mat, (void*)"Line" },
  { "0Modules/Geometry/Assign structural properties/Plate",
   (Fl_Callback *)read_sc_mat, (void*)"Surface" },
  { "0Modules/Geometry/Assign structural properties/Solid",
   (Fl_Callback *)read_sc_mat, (void*)"Volume" },
  ...
  { "0Modules/Periodicity/2D SG",
   (Fl_Callback *)geo_periodic_cb },
  { "0Modules/Periodicity/3D SG",
   (Fl_Callback *)geo_periodic_surf },

   // Prof. Yu
   { "0Modules/Mesh/Mesh control",
   (Fl_Callback *)mesh_options_cb },
   { "0Modules/Mesh/Generate 1D mesh/Refine lines(optl)",
   (Fl_Callback *)mesh_define_transfinite_line_cb },
   { "0Modules/Mesh/Generate 1D mesh/Generate",
   (Fl_Callback *)mesh_1d_cb },
   /*{ "0Modules/Mesh/Generate 2D mesh/Refine mesh/Refine points(optl)",
   (Fl_Callback *)mesh_define_length_cb },*/
   { "0Modules/Mesh/Generate 2D mesh/Refine lines(optl)",
   (Fl_Callback *)mesh_define_transfinite_line_cb },
   { "0Modules/Mesh/Generate 2D mesh/Refine surface(optl)",
   (Fl_Callback *)mesh_define_transfinite_surface_cb },
   { "0Modules/Mesh/Generate 2D mesh/Generate",             //"Reload", "Set order 2" are incorporated into Mesh 2D, Prof.Wenbin Yu; back to order 1
   (Fl_Callback *)mesh_2d_cb },

   { "0Modules/Mesh/Generate 3D mesh/Refine lines(optl)",
   (Fl_Callback *)mesh_define_transfinite_line_cb },
   { "0Modules/Mesh/Generate 3D mesh/Refine surface(optl)",
   (Fl_Callback *)mesh_define_transfinite_surface_cb },
   { "0Modules/Mesh/Generate 3D mesh/Refine volume(optl)",
   (Fl_Callback *)mesh_define_transfinite_volume_cb },

   {"0Modules/Mesh/Generate 3D mesh/Generate",             //"Reload", "Set order 2" are incorporated into Mesh 3D, Prof.Wenbin Yu; back to order 1
   (Fl_Callback *)mesh_3d_cb} ,
 /* {"0Modules/Mesh/Generate 3D mesh/Optimize 3D",
   (Fl_Callback *)mesh_optimize_cb} ,

#if defined(HAVE_NETGEN)
  {"0Modules/Mesh/Generate 3D mesh/Optimize 3D (Netgen)",
   (Fl_Callback *)mesh_optimize_netgen_cb} ,
#endif
  {"0Modules/Mesh/Set order 1",
  (Fl_Callback *)mesh_degree_cb, (void*)1},*/
  {"0Modules/Mesh/Set order 2",
  (Fl_Callback *)mesh_degree_cb, (void*)2},
  {"0Modules/Mesh/Size fields",                                         //Prof. Wenbin Yu
  (Fl_Callback *)field_cb },

  {"0Modules/Import SC file",                                      
  (Fl_Callback *)importSC2 },

   // Prof. Yu
  { "1SwiftComp/Homogenization/Beam Model",
  (Fl_Callback *)geometry_physical_add_cb, (void*)"HomoBeam" },
  { "1SwiftComp/Homogenization/Plate(Shell) Model",
  (Fl_Callback *)geometry_physical_add_cb, (void*)"HomoSP" },
  { "1SwiftComp/Homogenization/Solid Model",
  (Fl_Callback *)geometry_physical_add_cb, (void*)"HomoSolid" },
  { "1SwiftComp/Dehomogenization/Beam Model",
  (Fl_Callback *)geometry_physical_add_cb, (void*)"dehomoBeam" },
  { "1SwiftComp/Dehomogenization/Plate(Shell) Model",
  (Fl_Callback *)geometry_physical_add_cb, (void*)"dehomoSP" },
  { "1SwiftComp/Dehomogenization/Solid Model",
  (Fl_Callback *)geometry_physical_add_cb, (void*)"dehomoSolid" },
  /* { "1SwiftComp/Import SC file/Import file",
  (Fl_Callback *)importSC, (void*)"scfile" },
  { "1SwiftComp/Import SC file/Run dehomogenization",
  (Fl_Callback *)importSC, (void*)"scfile2" },*/

  { "2CalculiX/Write INP file",
  (Fl_Callback *)calculix_func, (void*)"writeINP" },
  { "2CalculiX/Import structural properties",
  (Fl_Callback *)calculix_func, (void*)"scmat" },
  { "2CalculiX/Define step",
  (Fl_Callback *)calculix_func, (void*)"step" },
  { "2CalculiX/Run",
  (Fl_Callback *)calculix_func, (void*)"run" },
  { "2CalculiX/Results",
  (Fl_Callback *)calculix_func, (void*)"res" },

  {"3Input control/Reload",
   (Fl_Callback *)geometry_reload_cb} ,
  {"3Input control/Edit file",
...
static void calculix_func(Fl_Widget *w, void *data)
{
  ...
  if(!data) return;
  std::string str((const char*)data);
  mesh_chec_cal mesh_b3;
  if(str == "writeINP"){
    if(mesh_b3.assign_flags==0) {fl_message("Please assign structural properites!"); return;}
    inpfile_create(name);
    add_mat_cal(name);
    mesh_b3.ch_inp(1);
  }
    ...
    if(mesh_b3.assign_flags==0) {fl_message("Please assign structural properites!"); return;}
    write_sc_mat(name,mesh_b3.ch_inp0(),mesh_b3.re());
    mesh_b3.ch_inp(0);
  }
  else if (str == "step"){
    open_cal_file(name);
    ...
    fl_message("Unknown parameters!");
  
}

void write_test_str(std::vector<std::string> &wrt1){
  std::ofstream write1;
  write1.open("testout.txt",std::ofstream::app);
  for(int i = 0; i < wrt1.size(); i++)
    write1 << wrt1[i] << "\n";
  write1.close();
}

void read_mat(std::vector<std::string> &iso_mat, std::vector<std::string> &ort_mat, \
	      std::vector<std::string> &ani_mat,std::vector<std::string> &matmz){
  std::string fname = GModel::current()->getFileName();
  ...
  while (getline(geofile, readoutline)){
    ...
    if (readoutline.find("name") != std::string::npos){
      //size_t found1 = readoutline.find_last_of( " " );
      readoutline.erase(0, 18);
      matmz.push_back(readoutline);
    }

    if(flags==1){
      std::stringstream tran_ss(readoutline);
      while (tran_ss >> temp){
	if(temp != "\")")
    ...
    if(flags==2){
      if(all_data[1]=="0"){
	iso_mat.push_back(all_data[0]);
	for(int i=0; i < 2; i++)
	  iso_mat.push_back(all_data[5+i]);
	all_data.clear();
	flags=0;
      }
      else if(all_data[1]=="1"){
	ort_mat.push_back(all_data[0]);
        for(int i=0; i < 9; i++)
	  ort_mat.push_back(all_data[5+i]);
	all_data.clear();
	flags=0;
      }
      else if(all_data[1]=="2"){
	ani_mat.push_back(all_data[0]);
	for(int i=0; i < 21; i++)
	  ani_mat.push_back(all_data[5+i]);
	all_data.clear();
	flags=0;
      }
      else{
	fl_message("Something wrong with material format in input file!");
	return;
      }
    }
  }
...
static void mat_edit_menu(Fl_Widget *w, void *data){
  ...
  if(str == "edit"){
    ...
    std::vector<std::string> iso_mat;
    std::vector<std::string> ort_mat;
    std::vector<std::string> ani_mat;
    std::vector<std::string> matmz;
    read_mat(iso_mat, ort_mat, ani_mat, matmz);
    for (int i=0; i<matmz.size(); i++)
      FlGui::instance()->geoContext20->bro->add(matmz[i].c_str());
    FlGui::instance()->geoContext20->show(0);
  }
  else{
    fl_message("Something wrong!");
    return;
  }
...
static void read_sc_mat(Fl_Widget *w, void *data)
{
  change_scelem_value(1);
  std::string str((const char*)data);
  if(str == "Line"){
    //add_physical(what, List1, GModel::current()->getFileName(), "101");
    action_point_line_surface_volume(20, 0, str.c_str());
  }
  else if(str == "Surface"){
  ...
  std::string name = GModel::current()->getFileName();
  std::ifstream kfile(scfile.c_str());
  std::string readline,temp;
  std::vector<std::string> scmat;
  int flags=0;
  while (getline(kfile, readline)){
    if(readline.find("Stiffness") != std::string::npos){
      flags=1;continue;}
      
    if(flags==1 && readline.find("----") == std::string::npos){
      std::stringstream tran_ss(readline);
      while (tran_ss >> temp){
	scmat.push_back(temp);}
      if (scmat.size()==16){
	flags=0;
	break;}
    }     
  ...
  writegeo.open(name.c_str(),std::ofstream::app);
  writegeo << "//*SCINPUT, ELSET=" << 100+scmat_flags <<", SCMODEL=B" <<"\n";
  for (int i =0; i < 4; i++)
    writegeo << "//" << scmat[4*i] << "," << scmat[4*i+1] << "," << scmat[4*i+2] << "," << scmat[4*i+3] << "\n";
  writegeo.close();

  mesh_chec_cal chec_ass;
  chec_ass.get_assig(1);
}
