...
void Ibeam_coor(int num_nodes, int num_elem); // Prof. Yu, recreate I beam msh file
void Bbeam_coor(int num_nodes, int num_elem); // Prof. Yu, recreate box beam msh file
                            ...
                            int dom1Num = 0, int dom2Num = 0)
{
  ...
  if(saveAll)
    ...
    //for(unsigned int j = 0; j < physicals.size(); j++){                                 //Prof.Wenbin Yu cancel loop because we do not want second physical: ply angle, 2014.10.24
	unsigned int j = 0;                                                                   //Prof.Wenbin Yu only write first physical: material
      ele->writeMSH2(fp, version, binary, ++num, elementary, physicals[j],
                     parentNum, dom1Num, dom2Num, &ghosts);
      if(parentNum) parentNum++;
    //}                                                                                  //Prof.Wenbin Yu cancel loop
  }
                       ...
                       int elementStartNum, int saveSinglePartition, bool multipleView)
{
  ...
  // Prof. Yu record number of elements and nodes for postprocessing
  std::string temp = GModel::current()->getFileName();
  std::string tename = temp + ".t2";
  std::ofstream postfile;
  postfile.open(tename.c_str());
  postfile << "numVertices " << numVertices << "\n" << "numElements " << numElements << "\n";
  postfile.close();


  // Prof. Yu, create postprocessing file
  int lastindex = temp.find_last_of(".");
  std::string tpostname = temp.substr(0, lastindex);
  std::string postview = tpostname + "VIEW" + ".msh";
  std::ofstream writepos;
  writepos.open(postview.c_str());
  writepos << "$MeshFormat\n";
  writepos << "2.2 0 8\n";
  writepos << "$EndMeshFormat\n";
  writepos << "$Nodes\n";
  writepos << numVertices << "\n";
  writepos.close();
...
// Prof. Wenbin Yu
  ...
  if(version >= 2.0){
	...
	// Prof. Yu , choose model type
  std::string tname = GModel::current()->getFileName();
  std::string tfilename = tname + ".t";
  ...
  while (getline(tfile, readout)) {
    ...
    if (readout.find(beam) != std::string::npos)
      {
	size_t pos = readout.find(beam);
	readout.erase(0, pos + 5);
	...
	if (BSPchar[0] == "0-Euler-Bernoulli beam model") fprintf(fp, "0		# model: 0-classical, 1-shear refined, 2-Vlasov, 3-Trapeze\n");
	else if (BSPchar[0] == "1-Timoshenko beam model") fprintf(fp, "1		# model: 0-classical, 1-shear refined, 2-Vlasov, 3-Trapeze\n");
	else if (BSPchar[0] == "2-Vlasov beam model") fprintf(fp, "2		# model: 0-classical, 1-shear refined, 2-Vlasov, 3-Trapeze\n");
	else if (BSPchar[0] == "3-beam model with the trapeze effect") fprintf(fp, "3		# model: 0-classical, 1-shear refined, 2-Vlasov, 3-Trapeze\n");
	fprintf(fp, "%s %s %s    #  initial	curvatures\n", BSPchar[1].c_str(), BSPchar[2].c_str(), BSPchar[3].c_str());
	fprintf(fp, "%s %s    #    oblique	parameters\n", BSPchar[4].c_str(), BSPchar[5].c_str());
	...
	if (BSPchar[0] == "0-Kirchhoff-Love shell") fprintf(fp, "0		# structural submodel\n");
	else if (BSPchar[0] == "1-Reissner-Mindlin shell") fprintf(fp, "1		# structural submodel\n");
	fprintf(fp, "%s %s    #  initial	curvatures\n", BSPchar[1].c_str(), BSPchar[2].c_str());
      }
	...
	if (BSPchar[0] == "0-Kirchhoff-Love plate") fprintf(fp, "0		# structural submodel\n");
	else if (BSPchar[0] == "1-Reissner-Mindlin plate") fprintf(fp, "1		# structural submodel\n");
	fprintf(fp, "0 0           # initial curvatures for plate\n");
      }
	...
	for (int i = 0; i < 4; i++){
	  if (thdchar[i] == "0-elastic")  fprintf(fp, "0 ");
	  else if (thdchar[i] == "1-thermoelastic") {fprintf(fp, "1 "); thermal_flags=1;}
	  else if (thdchar[i] == "2-conduction") {fprintf(fp, "2 "); thermal_flags = 2;}
	  else if (thdchar[i] == "3-piezoelectric/piezomagnetic") fprintf(fp, "3 ");
	  else if (thdchar[i] == "4-thermoepiezoelectric/thermopiezomagnetic") fprintf(fp, "4 ");
	  else if (thdchar[i] == "5-piezoelectromagnetic") fprintf(fp, "5 ");
	  else if (thdchar[i] == "6-thermopiezoelectromagnetic") fprintf(fp, "6 ");
	  else if (thdchar[i] == "0-regular elements") fprintf(fp, "0 ");
	  else if (thdchar[i] == "1-one dimension degenerated") fprintf(fp, "1 ");
	  else if (thdchar[i] == "2-two dimension degenerated") fprintf(fp, "2 ");
	  else if (thdchar[i] == "0-global coordinate") fprintf(fp, "0 ");
	  else if (thdchar[i] == "1-material coordinate") fprintf(fp, "1 ");
	  else if (thdchar[i] == "0-uniform") fprintf(fp, "0             # analysis elem_flag trans_flag temp_flag\n");
	  else if (thdchar[i] == "1-not uniform") fprintf(fp, "1             # analysis elem_flag trans_flag temp_flag\n");
	  //else  unitcellarea = thdchar[i];
	}
  ...
  //Prof. Yu, check dimension of SG 
  std::ifstream DSG(tname.c_str());
  std::string Dline;
  std::string surface = "Physical Surface";
  std::string volume = "Physical Volume";
  while (getline(DSG, Dline)){
    if (Dline.find(volume) != std::string::npos){
      unitcelldi = 3;
      break;
    }
  }
  DSG.close();
  if (unitcelldi != 3) {
    std::ifstream DSG2(tname.c_str());
    std::string Dline2;
    while (getline(DSG2, Dline2)){
      if (Dline2.find(surface) != std::string::npos){
	unitcelldi = 2;
	break;
      }
    }
  ...
  // Prof. Yu, check layer-id
  std::string layer = "nlayers";
  std::string temp_l1, temp_l2;
  int nlayers = 0;
  int flagsL = 0;
  std::vector<std::string> layer_id;
  std::ifstream layerID(tname.c_str());
  while (getline(layerID, Dline)){
    if (Dline.find("layers_end") != std::string::npos) break;
    if (flagsL == 1){
      layer_id.push_back(Dline.erase(0,3));
    }
    
    if (Dline.find(layer) != std::string::npos){
      flagsL = 1;
      std::istringstream first(Dline);
      first >> temp_l1 >>  temp_l2 >> nlayers;
    }
  }
  layerID.close();
  
  fprintf(fp, "%d ", unitcelldi);
  
  fprintf(fp, "%d %d ", numVertices, numElements);                 //Prof.Wenbin Yu
  
  fprintf(fp, "%d ", numPhysicalNames());	
  ...
  if (nlayers != 0)
    fprintf(fp, " 0 %d         # nSG nnode nelem nmate nslave nlayer\n\n", nlayers);
  else
    fprintf(fp, " 0 0         # nSG nnode nelem nmate nslave nlayer\n\n");                 //Prof.Wenbin Yu
  
  
  
  // Prof. Yu, write nodes into postprocessing file
  writepos.open(postview.c_str(), std::ofstream::app);
  
  //double geonodes[numVertices][3];
  //int row = 0;
  for(unsigned int i = 0; i < entities.size(); i++)
    for(unsigned int j = 0; j < entities[i]->mesh_vertices.size(); j++){
		...
		if (entities[i]->mesh_vertices[j]->getIndex()>0){
			  writepos << entities[i]->mesh_vertices[j]->getIndex() << " " << entities[i]->mesh_vertices[j]->x() << " " << entities[i]->mesh_vertices[j]->y() << " " << entities[i]->mesh_vertices[j]->z() << "\n";
 ...
 // for 3D unit cell, do not write 2d element mesh, Prof. Wenbin Yu
  if (unitcelldi == 2){
  // triangles
  for(fiter it = firstFace(); it != lastFace(); ++it)
  ...
  //Prof. Yu, read materials properties
   std::string mshfile = CTX::instance()->outputFileName;
   mshfile = GetDefaultFileName(FORMAT_MSH);
   std::ifstream geofile(temp.c_str());
   ...
   while (getline(geofile, readoutline)) {
	...
	std::ifstream area_check(tname.c_str());
	std::string a_check;
	int a_flags=0;
	int Ibeam_flags=0;
	int Bbeam_flags=0;
	double Unitarea;
	std::string tstr;
	while (getline(area_check, a_check)){
	  if (a_check.find("area") != std::string::npos){
	    a_flags = 1;
	    //std::istringstream first(a_check);
	    //first >> tstr >> Unitarea;
	    Unitarea = 1;
	    break;
	  }
	  if (a_check.find("Composites I beam") != std::string::npos){
	    Ibeam_flags=1;
	  }
	  if (a_check.find("Composites box beam") != std::string::npos){
	    Bbeam_flags=1;
	  }
	  if (a_check.find("beam section") != std::string::npos){
	    a_flags = 1;
	    Unitarea = 1;
	    break;
	  }
	...
	if (a_flags == 0){
	  CheckGEO area_vol;
	  Unitarea = area_vol.uni_area(tname.c_str());
	}
  ...
  ///////////////////////////////////////////////////////////////////////////////////
  // Prof. Yu, add element coordinates
  if (Ibeam_flags == 1){
    Ibeam_coor(numVertices, numElements);
  }
  if (Bbeam_flags == 1){
    Bbeam_coor(numVertices, numElements);
  }
  return 1;
}
...
void Ibeam_coor(int num_nodes, int num_elem){
  int count=0;
  std::vector<int> final;
  std::vector<int> wall_id;
  std::string filename = GModel::current()->getFileName();
  std::ifstream get_infor(filename.c_str());
  std::string readout;
  while (getline(get_infor,readout)){
    if (readout.find("//EC")!= std::string::npos){
      std::stringstream s(readout);
      int i1, i2;
      std::string t1;
      if (s >> t1 >> i1 >> i2){
	final.push_back(i1);
	wall_id.push_back(i2);
      }
    }
  }
  get_infor.close();
  ...
  std::string mshfile = filename.substr(0, lastindex)+".msh";
  std::string b_coor = "1 0 0 0 -1 0 0 0 0";
  std::string u_coor = "1 0 0 0 1 0 0 0 0";
  std::string m_coor = "1 0 0 0 0 -1 0 0 0";
  std::vector<std::string> e1e2e3;
  std::vector<std::string> firstpart;
  std::vector<std::string> lastpart;
  e1e2e3.push_back(b_coor);
  e1e2e3.push_back(u_coor);
  e1e2e3.push_back(m_coor); 
  std::string temp;
  std::vector<std::vector<int> > elem_cnt;
  std::vector<int> elem_coor;
  std::ifstream data2(mshfile.c_str());
  int readflags= num_nodes+7;
  while (getline(data2,readout)){
    if (count < readflags)
      firstpart.push_back(readout);
    else if (count >= readflags && count < readflags+num_elem){
      std::vector<int> reco;
      std::stringstream tran_ss(readout);
      while (tran_ss >> temp)
	reco.push_back(atoi(temp.c_str()));
      elem_coor.push_back(wall_id[reco[1]-1]);
      reco[1] = final[reco[1]-1];    
      elem_cnt.push_back(reco);
    }
    else
      lastpart.push_back(readout);
    count++;
  }
  data2.close();
  
  std::remove(mshfile.c_str());
  std::ofstream writemsh;
  std::string Ioutput = mshfile;
  writemsh.open(Ioutput.c_str());
  for (int i = 0; i < firstpart.size(); i++)
    writemsh << firstpart[i] << "\n";  
  
  for (int i = 0; i < elem_cnt.size(); i++){
    for (int j = 0; j < elem_cnt[i].size(); j++)
      writemsh << elem_cnt[i][j] << " ";
    writemsh << "\n";
  }
    
  for (int i = 0; i < elem_coor.size(); i++)
    writemsh << i+1 << " " << e1e2e3[elem_coor[i]-1] << "\n";
...
void Bbeam_coor(int num_nodes, int num_elem){
  int count=0;
  std::vector<int> final;
  std::vector<int> wall_id;
  std::string filename = GModel::current()->getFileName();
  ...
  std::string mshfile = filename.substr(0, lastindex)+".msh";
  std::string u_coor = "1 0 0 0 1 0 0 0 0";
  std::string l_coor = "1 0 0 0 0 1 0 0 0";
  std::string b_coor = "1 0 0 0 -1 0 0 0 0";
  std::string r_coor = "1 0 0 0 0 -1 0 0 0";
  std::vector<std::string> e1e2e3;
  std::vector<std::string> firstpart;
  std::vector<std::string> lastpart;
  e1e2e3.push_back(u_coor);
  e1e2e3.push_back(l_coor);
  e1e2e3.push_back(b_coor);
  e1e2e3.push_back(r_coor);
  std::string temp;
  std::vector<std::vector<int> > elem_cnt;
  std::vector<int> elem_coor;
  std::ifstream data2(mshfile.c_str());
