...
//Prof. Yu, check element types
int check_dim(){
	std::string geoname = GModel::current()->getFileName();
	std::string readline;
	std::ifstream Dim(geoname.c_str());
	int flagsDim=0;
	while (getline(Dim,readline)){
		if (readline.find("Volume(") != std::string::npos) {
			return 3;
		}
		if (readline.find("Surface{") != std::string::npos) {
			return 3;
		}
		if (readline.find("Surface(") != std::string::npos) {
			flagsDim = 2;
